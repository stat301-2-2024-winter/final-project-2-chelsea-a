...
This report aims to predict student enrollment outcomes in a Portugal higher education institution using demographic, academic, and other relevant factors. Predicting enrollment outcomes is crucial for educational institutions to implement early intervention strategies and support student success. The primary objective is to understand the factors influencing enrollment outcomes such as dropping out, graduating, and continuing enrollment through the construction of an accurate multiclass classification prediction model.
...
![Correlation Plot](figures/correlation_plot.png){#fig-cor-plot}
...
Next, I plotted ROC curves for each class to give insights into the model's ability to distinguish between different classes. The ROC curve in @fig-roc-curve also shows that the model is very well at predicting graduates, good at predicting dropouts, and not so well at predicting students who remained enrolled. 

![Roc Curve](figures/student_curve.png){#fig-roc-curve}
...
An interesting variable that was also an important predictor was the mother's occupation variable. Initially, I believed that this variable would not be very useful considering how many occupations there are, however, it proved to be important in my winning boosted tree model. 

Although the dataset pertains specifically to a higher education institution in Portugal, valuable findings can be taken from it and applied to institutions around the world. Notably, there is a very strong correlation between variables concerning curricular units that are enrolled, credited, approved, and evaluated. Particularly noteworthy is the predictive power associated with the number of curricular units approved in the second semester. Overall, higher education institutions could use implement policies such as academic support initiatives and targeted measures to increase graduation rates for their students.
